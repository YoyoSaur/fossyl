...
export async function createUser(name: string, email: string): Promise<userRepo.User> {
  return userRepo.create({ name, email });
}

export async function updateUser(id: string, data: { name?: string }): Promise<userRepo.User> {
